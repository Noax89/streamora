...
export const MovieCard = memo(({ movie, className }: MovieCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn('relative group min-w-40 md:min-w-55 lg:min-w-65 aspect-2/3 rounded-xl overflow-hidden shadow-lg border border-white/5', className)}
    >
      <Link to={`/movie/${movie.id}`} className="block w-full h-full">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
          ...
          <h3 className="text-sm font-bold text-white mb-3 line-clamp-1">{movie.title}</h3>
